...
function blink(stones_map) {
  ...
  new_stones_map[1]=( stones_map[1] || 0 ) + stones_map[0];
  ...
  for (const key in stones_map) {
      ...
      if (length%2==0) {
        new_stones_map[Number(engraving.slice(-length/2))] = ( new_stones_map[Number(engraving.slice(-length/2))] || 0) + 1;
        new_stones_map[ Number(engraving.slice(0, length/2)) ] = ( new_stones_map [ Number(engraving.slice(0, length/2))  ] || 0) + 1;
      } else {
