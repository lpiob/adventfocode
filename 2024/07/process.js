  ...
  for (let i=0; i < totalVariations; i++) {
    ...
    while (ni < numbers.length) {
      ...
      if (ni>0) { //(ni+1)%2==0 || ni+1==numbers.length) {
        variation.push(operators[(Math.floor(i / Math.pow(3, oc)) % operators.length)]);
        oc++;
      };
      ni++;
    }
    ...
    let evres=evaluateREN(variation);
    if (evres==result) { 
      return true;
    }
        ...
        if (!isNaN(token)) {
            ...
            switch (token) {
                ...
                case '*':
                    ...
                    break;
                case '||':
                    stack.push(Number(String(a)+String(b)));
                    break;
                default:
