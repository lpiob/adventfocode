...
console.log(i, corruptedQueue.length);
i=ns;
while (i<corruptedQueue.length) {
  let ret=pqsolve();
  let c=corruptedQueue[i];
    map[c[0]][c[1]]='#';
  if (ret[0]<Infinity) {
    console.log("After corrupting",i,"bytes, we found a solution with minimal number of steps: ", ret[0]);
  } else {
    c=corruptedQueue[i-1];

    console.log("Maze unsolvable after corrupting",i,"bytes");
    console.log("Last corrupted memory block",c.reverse());
    break;
  };
  i++;
};
